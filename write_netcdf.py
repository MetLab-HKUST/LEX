...
def save2nc(phys_state, sfc_others, grids, sprint_i, model_time):
    ...
    (theta, u, v, w, pip, qv) = phys_state
    # pip is the previous step pi' for pseudo-incompressible equation, but current pi' for explicit solver
    ...
    theta_1 = nc_file.createVariable("theta", np.float32, ("time", "x", "y", "z"), fill_value=1.0e36, zlib=True, complevel=2)
    theta_1.long_name = "theta for the current time"
    theta_1[:, :, :, :] = np.reshape(theta[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz],
                                     (1, x_size, y_size, z_size))

    pip_1 = nc_file.createVariable("pip", np.float32, ("time", "x", "y", "z"), fill_value=1.0e36, zlib=True, complevel=2)
    pip_1.long_name = "pip for the current time"
    pip_1[:, :, :, :] = np.reshape(pip[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz],
                                   (1, x_size, y_size, z_size))

    qv_1 = nc_file.createVariable("qv", np.float32, ("time", "x", "y", "z"), fill_value=1.0e36, zlib=True, complevel=2)
    qv_1.long_name = "qv for the current time"
    qv_1[:, :, :, :] = np.reshape(qv[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz], (1, x_size, y_size, z_size))

    u_1 = nc_file.createVariable("u", np.float32, ("time", "x4u", "y", "z"), fill_value=1.0e36, zlib=True, complevel=2)
    u_1.long_name = "u for the current time"
    u_1[:, :, :, :] = np.reshape(u[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz], (1, x4u_size, y_size, z_size))

    v_1 = nc_file.createVariable("v", np.float32, ("time", "x", "y4v", "z"), fill_value=1.0e36, zlib=True, complevel=2)
    v_1.long_name = "v for the current time"
    v_1[:, :, :, :] = np.reshape(v[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz], (1, x_size, y4v_size, z_size))

    w_1 = nc_file.createVariable("w", np.float32, ("time", "x", "y", "z4w"), fill_value=1.0e36, zlib=True, complevel=2)
    w_1.long_name = "w for the current time"
    w_1[:, :, :, :] = np.reshape(w[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz], (1, x_size, y_size, z4w_size))

    info_1 = nc_file.createVariable("info", np.float32, "time", fill_value=1.0e36, zlib=True, complevel=2)
    info_1.long_name = "exit code of the Poisson equation solver"
    info_1[:] = info

    if nl.pic_opt:
        pc_1 = nc_file.createVariable("pip_const", np.float32, "time", fill_value=1.0e36, zlib=True, complevel=2)
        pc_1.long_name = "pi' correction constant for the current time"
        pc_1[:] = pip_const

    if nl.sfc_opt:
        tau_x_1 = nc_file.createVariable("tau_x", np.float32, ("time", "x", "y"), fill_value=1.0e36, zlib=True, complevel=2)
        tau_x_1.long_name = "tau_x for the current time"
        tau_x_1[:, :, :] = np.reshape(tau_x, (1, x_size, y_size))

        tau_y_1 = nc_file.createVariable("tau_y", np.float32, ("time", "x", "y"), fill_value=1.0e36, zlib=True, complevel=2)
        tau_y_1.long_name = "tau_y for the current time"
        tau_y_1[:, :, :] = np.reshape(tau_y, (1, x_size, y_size))

        sen_1 = nc_file.createVariable("sen", np.float32, ("time", "x", "y"), fill_value=1.0e36, zlib=True, complevel=2)
        sen_1.long_name = "sensible heat for the current time"
        sen_1[:, :, :] = np.reshape(sen, (1, x_size, y_size))

        evap_1 = nc_file.createVariable("evap", np.float32, ("time", "x", "y"), fill_value=1.0e36, zlib=True, complevel=2)
        evap_1.long_name = "evaporation for the current time"
        evap_1[:, :, :] = np.reshape(evap, (1, x_size, y_size))

        t_ref_1 = nc_file.createVariable("T_ref", np.float32, ("time", "x", "y"), fill_value=1.0e36, zlib=True, complevel=2)
        t_ref_1.long_name = "temperature at reference height (2m) for the current time"
        t_ref_1[:, :, :] = np.reshape(t_ref, (1, x_size, y_size))

        q_ref_1 = nc_file.createVariable("q_ref", np.float32, ("time", "x", "y"), fill_value=1.0e36, zlib=True, complevel=2)
        q_ref_1.long_name = "mixing ratio at reference height (2m) for the current time"
        q_ref_1[:, :, :] = np.reshape(q_ref, (1, x_size, y_size))

        u10_1 = nc_file.createVariable("u10", np.float32, ("time", "x", "y"), fill_value=1.0e36, zlib=True, complevel=2)
...
def save2nc_base(base_state, grids):
    ...
    x3d, y3d, z3d, _, _, _, _, _, _, _ = grids
