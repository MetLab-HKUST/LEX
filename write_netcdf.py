...
def save2nc(phys_state, sfc_others, grids, sprint_i, model_time):
    ...
    qv_2[:, :, :, :] = np.reshape(qv_next[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz], (1, x_size, y_size, z_size))
    ...
    u_2[:, :, :, :] = np.reshape(u_next[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz], (1, x4u_size, y_size, z_size))
    ...
    v_2[:, :, :, :] = np.reshape(v_next[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz], (1, x_size, y4v_size, z_size))
    ...
    w_2[:, :, :, :] = np.reshape(w_next[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz], (1, x_size, y_size, z4w_size))
...
def save2nc_base(base_state, grids):
    """ Write data to a NetCDF file """
    filename = "experiments/lex_reference_state.nc"
    nc_file = nc4.Dataset(filename, mode="w", format="NETCDF4")
    nc_file.createDimension("x", nl.nx)
    nc_file.createDimension("y", nl.ny)
    nc_file.createDimension("z", nl.nz)
