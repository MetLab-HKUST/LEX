...
def ws_rk3_big_step(phys_state, base_state, grids, model_opt):
    """ Integration using RK3 method for one big step

    Three-stage RK3 based on Wicker and Skamarock (2002).
    """
    solver_opt = model_opt[0]

    xi0 = phys_state
    xi1, _, sfc_others, heating, sgs_tend = one.rk_sub_step0(xi0, xi0, base_state, grids, model_opt, nl.dt/3.0)
    xi2, _ = one.rk_sub_step1(xi0, xi1, base_state, grids, heating, sfc_others, sgs_tend, model_opt, nl.dt/2.0)
    xi3, _ = one.rk_sub_step2(xi0, xi2, base_state, grids, heating, sfc_others, sgs_tend, model_opt, nl.dt)

    if solver_opt == 1:
        theta_next, u_next, v_next, w_next, pip_now, qv_next = xi3
        phys_state = (theta_next, u_next, v_next, w_next, pip_now, qv_next)
    else:
        theta_next, u_next, v_next, w_next, pip_next, qv_next = xi3
...
@partial(jax.jit, static_argnames=['model_opt'])
def ws_rk3_sprint_dl_train(theta_now, u_now, v_now, w_now, pip, qv_now, base_state, grids, model_opt, dl_params, scaling_params):
    """ Integration using RK3 method for sprint_n big steps, with deep learning correction
    ...
    for i in range(nl.sprint_n):
        ...
        phys_state_now = (theta_now, u_now, v_now, w_now, pip, qv_now)
        theta_prev = theta_now
        phys_state_next, _ = ws_rk3_big_step(phys_state_now, base_state, grids, model_opt)
        theta_now, u_now, v_now, w_now, pip, qv_now = phys_state_next
        # DL corrections
        ...
        pip_stack = pip_stack.at[i, :, :, :].set(pip[nl.ngx:-nl.ngx, nl.ngy:-nl.ngy, nl.ngz:-nl.ngz])
...
@partial(jax.jit, static_argnames=['model_opt'])
def ws_rk3_sprint_dl_train_vmap(theta_batch, u_batch, v_batch, w_batch, pip_batch, qv_batch,
                                base_state, grids, model_opt, dl_params, scaling_params):
    theta_stack, u_stack, v_stack, w_stack, pip_stack, qv_stack = jax.vmap(
        ws_rk3_sprint_dl_train, (0, 0, 0, 0, 0, 0, None, None, None, None, None),
...
@partial(jax.jit, static_argnames=['model_opt'])
def ws_rk3_sprint(phys_state, base_state, grids, model_opt):
    """ Integration using RK3 method for sprint_n big steps """
    for i in range(nl.sprint_n):
        phys_state, sfc_others = ws_rk3_big_step(phys_state, base_state, grids, model_opt)

    return phys_state, sfc_others


@partial(jax.jit, static_argnames=['model_opt'])
def ws_rk3_sprint_dl(phys_state, base_state, grids, model_opt, dl_params, scaling_params):
    """ Integration using RK3 method for sprint_n big steps, with deep learning correction

    This is the application, not training. For training the DL model, use ws_rk3_sprint_dl_train
    """
    theta_min, theta_max, u_min, u_max, v_min, v_max, w_min, w_max, qv_min, qv_max, _, _, _, _ = scaling_params
    phys_state_now = phys_state
    for i in range(nl.sprint_n):
        # dynamical core integration
        phys_state_next, sfc_others = ws_rk3_big_step(phys_state_now, base_state, grids, model_opt)
        theta_now, u_now, v_now, w_now, pip, qv_now = phys_state_next
        # DL corrections
        phys_state_stack = dlm.phys_state_tuple2array(phys_state_next, scaling_params)
        # This stack is the input for the DL model
