...
LES has two dynamical cores. One adopts the pseudo-incompressible equations and the other uses fully compressible equations. It uses the Weighted Essentially Nonoscillatory (WENO) schemes to calculate fluxes and a third-order Runge-Kutta (RK3) method to integrate in time. The following figure is our benchmark simulation of a simple warm bubble case. Its accuracy is the same as that of the fully compressible Cloud Model 1 (CM1) for this simple case.
